...
async function getPage(pageNum)
{
    ...
    let {data} = await axios.get(`https://api.themoviedb.org/3/discover/tv?api_key=9b3a5414c1198ebced482e012a215ab5&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${pageNum}`);
      ...
      <div className="col-xlg-2 col-lg-3 col-md-4 col-sm-6 col-12 py-2 " key={i}>
        <Link to={`/tvdetails/${movie.id}`}>
          ...
          <div className='overflow-hidden '>
            ...
            <h6 className={`${style.homeTitle}`} >{movie.name}</h6>
        </Link>
      </div>
      )
    }
